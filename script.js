const appData = {
    ...
    dragStart(event) { this.dragging = { startX: (event.touches ? event.touches[0].clientX : event.clientX) - this.dataBlock.x, startY: (event.touches ? event.touches[0].clientY : event.clientY) - this.dataBlock.y }; },
    dragMove(event) { if (this.dragging) { this.dataBlock.x = (event.touches ? event.touches[0].clientX : event.clientX) - this.dragging.startX; this.dataBlock.y = (event.touches ? event.touches[0].clientY : event.clientY) - this.dragging.startY; } },
    dragEnd() { this.dragging = null; },
    resizeStart(event) { event.stopPropagation(); this.resizing = { startX: (event.touches ? event.touches[0].clientX : event.clientX), startY: (event.touches ? event.touches[0].clientY : event.clientY), initialW: this.dataBlock.width, initialH: this.dataBlock.height }; },
    resizeMove(event) { if (this.resizing) { this.dataBlock.width = this.resizing.initialW + ((event.touches ? event.touches[0].clientX : event.clientX) - this.resizing.startX); this.dataBlock.height = this.resizing.initialH + ((event.touches ? event.touches[0].clientY : event.clientY) - this.resizing.startY); } },
    resizeEnd() { this.resizing = null; },
    downloadImage() { alert('Funcionalidade de download a ser implementada.'); },
    initChart() {
        const timeToSeconds = (timeStr) => {
            const parts = timeStr.split(':'); return (+parts[0]) * 3600 + (+parts[1]) * 60 + (+parts[2]);
        };
