document.addEventListener('alpine:init', () => {
    Alpine.data('athleteDashboard', function () {
        return {
            ...
            dragging: null, // Objeto para guardar estado do arraste
            resizing: null, // Objeto para guardar estado do redimensionamento
            ...
            get summary() {
                ...
                if (resultados.length === 0) return { totalPoints: 0, record: { tempo: '-', prova: '-' }, totalRaces: 0, since: '-' };
                const record = resultados.reduce((best, current) => (timeToSeconds(current.tempo) < timeToSeconds(best.tempo) ? current : best));
                const oldestRace = [...resultados].sort((a, b) => new Date(a.data) - new Date(b.data))[0];
                return {
                    totalPoints: resultados.reduce((sum, r) => sum + r.pontos, 0),
                    record: record, totalRaces: resultados.length,
                    since: new Date(oldestRace.data).toLocaleDateString('pt-BR', { year: 'numeric', month: 'short', timeZone: 'UTC' })
                };
            },
            ...
            dragStart(event) {
                this.dragging = {
                    ...
                    initialX: this.dataBlock.x, initialY: this.dataBlock.y
                };
            },
            dragMove(event) {
                ...
                this.dataBlock.x = this.dragging.initialX + (currentX - this.dragging.startX);
                this.dataBlock.y = this.dragging.initialY + (currentY - this.dragging.startY);
            },
            ...
            downloadImage() {
                alert('Funcionalidade de download será implementada assim que o layout for 100% validado!');
            },
            ...
            initChart(canvasElement) {
                ...
                new Chart(canvasElement, {
                    ...
                    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { color: 'rgba(255,255,255,0.7)' } }, x: { ticks: { color: 'rgba(255,255,255,0.7)' } } } }
                });
            }
        }
    });
